...
const addUser = async (req, res, next) => {
  try {
    ...
    var token = jwt.sign(
      { id: docRef._key.path.segments[1], role: ["Free"] },
      config.secret,
      {
    ...
    link =
      "http://" +
      req.get("host") +
      `/api/user/${docRef._key.path.segments[1]}/verify?id=${token}`;
    mailOptions = {
    ...
    res.send({
      message: "Gmail hesabinizi tesdiq edin",
      host: host,
      mail: mail,
    });
  } catch (error) {
...
const login = async (req, res, next) => {
  try {
    ...
    var token = jwt.sign(
      { id: User.id, role: User.data().claims },
      config.secret,
      {
        expiresIn: 86400 * 30, // expires in 30 day
      }
